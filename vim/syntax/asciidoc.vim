...
"     `abcd'
"
"   Will match as passthru until the next paragrapg or backtick.
...
syn region asciidocMonospace start="\(^\|[ \t(\[.,\-]\)\@<=`\([)]\)\@!" end="\(`\([ \t)\],.?!;:\-]\|$\)\@=\|^$\)"
" NOTE: can't use this because it matches (e.g.) X + Y.
...
setlocal formatoptions=tcqn autoindent
...
" FIXME: the latex suite does this better, also this should be local
" imap "" ``''<ESC>hi
" map "" i""
